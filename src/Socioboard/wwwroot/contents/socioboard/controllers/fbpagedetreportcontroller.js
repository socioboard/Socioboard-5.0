...
SocioboardApp.controller('FbpagedetreportController', function ($rootScope, $scope, $http, $timeout, apiDomain) {
    //alert('helo');
    $scope.$on('$viewContentLoaded', function () {
        ...
        //for fbpage daily report details-----strat
        $scope.getfbpagedetils = function (days) {
        ...
        $scope.generateChartforlikes = function (days) {
            ...
            var startDate = new Date((Date.now() - (days * 86400000))) / 1000;
            angular.forEach($scope.fbpagedet, function (value, key) {
                if (value.date > startDate) {
                    //totalLikes = totalLikes + parseInt(value.totalLikes);
                    //talkingAbout = talkingAbout;//+ parseInt(value.talkingAbout);
