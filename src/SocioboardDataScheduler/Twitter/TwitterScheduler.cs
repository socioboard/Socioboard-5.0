    ...
    public class TwitterScheduler
    {
        ...
        public static string ComposeTwitterMessage(string message, string profileid, long userid, string picurl, bool isScheduled, DatabaseRepository dbr, Domain.Socioboard.Models.TwitterAccount TwitterAccount, Domain.Socioboard.Models.ScheduledMessage schmessage, Domain.Socioboard.Models.User _user)
        {
            ...
            if (!string.IsNullOrEmpty(picurl))
            {
                try
                {
                    ...
                    if (picurl.Contains("mp4"))
                    {
                        ...
                        byte[] img = webClient.DownloadData(picurl);
                        rt = videoUploading(img, message, objTwitterAccount.oAuthToken, objTwitterAccount.oAuthSecret);
                    }
                    else
                    {
            ...
            if (!string.IsNullOrEmpty(ret) || rt == true)
            {
                ...
                {
                    string sucResponse = SendEmailbyelastic("", "", "Socioboard updates", _user.EmailId, "", "", _user.FirstName, schmessage.localscheduletime, true);
                }
            ...
            else
            {
                ...
                {
                    string falResponse = SendEmailbyelastic("", "", "Socioboard updates", _user.EmailId, "", "", _user.FirstName, schmessage.localscheduletime, false);
                }
            }
        ...
        public static bool videoUploading(byte[] binary, string tweetmessage, string accesstoken, string tokensecret)
        {
            try
            {
                Tweetinvi.Auth.SetUserCredentials(Helper.AppSettings.twitterConsumerKey, Helper.AppSettings.twitterConsumerScreatKey, accesstoken, tokensecret);
        ...
        public static string SendEmailbyelastic(string from, string html, string subject, string tomail, string UserId, string Pass, string Name, DateTime time, bool schStatus)
        {
            try
            {
                ...
                string API_KEY = "3eb5a724-e696-40fc-8d4f-7b33f488f3a3"; /*"7ea2a193-31d4-407e-bed9-8c652735862c";*/
