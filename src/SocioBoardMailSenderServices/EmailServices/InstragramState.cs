...
namespace SocioBoardMailSenderServices.EmailServices
{
    public class InstragramState
    {
        ...
        public static List<Domain.Socioboard.Models.Mongo.InstagramFeed> GetImagePosts(string profileId, DateTime start, DateTime end)
        {
            MongoRepository instagramFeedRepo = new MongoRepository("InstagramFeed");
            DateTime dayStart = new DateTime(start.Year, start.Month, start.Day, 0, 0, 0, DateTimeKind.Utc);
            DateTime dayEnd = new DateTime(end.Year, end.Month, end.Day, 23, 59, 59, DateTimeKind.Utc);
        ...
        public static List<Domain.Socioboard.Models.Mongo.InstagramComment> GetInstagramPostComments(string profileId, DateTime start, DateTime end)
        {
        ...
        public static List<Domain.Socioboard.Models.Mongo.InstagramPostLikes> GetInstagramPostLikes(string profileId, DateTime start, DateTime end)
        {
        ...
        public static List<Domain.Socioboard.Models.Mongo.MongoMessageModel> GetInstagramFollwerFollowing(string profileId, DateTime start, DateTime end)
        {
            MongoRepository mongorepo = new MongoRepository("MongoTwitterMessage");
            DateTime dayStart = new DateTime(start.Year, start.Month, start.Day, 0, 0, 0, DateTimeKind.Utc);
            DateTime dayEnd = new DateTime(end.Year, end.Month, end.Day, 23, 59, 59, DateTimeKind.Utc);
            var ret = mongorepo.Find<Domain.Socioboard.Models.Mongo.MongoMessageModel>(t => t.profileId == profileId && t.messageTimeStamp <= SBHelper.ConvertToUnixTimestamp(dayEnd) && t.messageTimeStamp >= SBHelper.ConvertToUnixTimestamp(dayStart) && (t.type == Domain.Socioboard.Enum.MessageType.InstagramFollower || t.type == Domain.Socioboard.Enum.MessageType.InstagramFollowing));
            var task = Task.Run(async () => {
                return await ret;
            });
            IList<Domain.Socioboard.Models.Mongo.MongoMessageModel> lstMongoTwitterMessage = task.Result.ToList();
