import Validator from './socialAccount.validate.js';
import {
...
class SocialAccountService {
  ...
  async getProfileRedirectUrl(req, res, next) {
    try {
      ...
      let team = await teamLibs.isValidTeam(
      ...
      let redirectUrl = '';
      let resultJson = '';
      let userScopeAvailableNetworks = req.body.userScopeAvailableNetworks;
      let state = {
        teamId: teamId,
        network: network,
          ...
          if (userScopeAvailableNetworks.includes('4')) {
            return this.twtConnect
              ...
              .then(response => {
                ...
                let twtencryptedState = this.authorizeServices.encrypt(
                  JSON.stringify(state)
                );
  ...
  async addSocialProfile(req, res, next) {
    try {
      ...
      if (error) return ValidateErrorResponse(res, error.details[0].message);
      var networkId = this.coreServices.networks[req.query.network];
      var queryInputs = req.query;
      let userId = req.body.userScopeId;
      let userName = req.body.userScopeName;
      ...
      switch (req.query.network) {
          ...
          if (userScopeAvailableNetworks.includes('4')) {
            ...
            return this.twtConnect
              ...
              .then(updateDetails => {
                ...
                var twitterInsightPostModelObject =
              ...
              .then(() => {
                if (flag == 1) {
                  try {
                    let follres = this.twtConnect.followTwitterId(
                      result.profileDetails.access_token,
                      result.profileDetails.refresh_token,
        ...
        case 'LinkedIn':
          ...
          } else ErrorResponse(res, 'Not available.');
          break;
        case 'Instagram':
          if (userScopeAvailableNetworks.includes('5')) {
  ...
  async deleteSocialProfile(req, res, next) {
    try {
      ...
      if (error) return ValidateErrorResponse(res, error.details[0].message);
      let response = await teamLibs.deleteSocialProfile(
        req.body.userScopeId,
        AccId
      );
