...
import {
  DELETE_TEAM_SOCIAL_PROFILE, SERVER,
} from '../constants';
...
const user = {
  username: 'testDeleteTeamSocialProfile',
  email: 'testDeleteTeamSocialProfile@testtest.com',
...
describe('Add other team account', () => {
  test('If access token is not provided', async () => {
    await request(SERVER)
      .delete(DELETE_TEAM_SOCIAL_PROFILE)
  ...
  test('If access token and query params are provided', async () => {
    ...
    await request(SERVER)
      ...
      .set('x-access-token', accessToken)
      .expect(200)
      .expect(({ body }) => expect(body.code).toBe(402));
  });
});
