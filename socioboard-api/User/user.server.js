...
// import recent-visited from './middleware/recent-visited.middleware.js'
const app = express();
...
const startServer = () =>
  new Promise((resolve, reject) => {
    const port = config.get('user_socioboard.port');

    server.listen(port, () => {
      Logger.info(
        `service listening on ${config.get('user_socioboard.host_url')} with ${
