...
import {
  SuccessResponse, CatchResponse, ErrorResponse, ValidateErrorResponse,
...
class YoutubeUploadService {
  ...
  async uploadVideo(req, res) {
    try {
      ...
      const uploadedVideoCountPer24hrs = await youTubeModel.getUploadedVideoCountPer24hrs(req.body.userScopeId, accountId, teamId);

      if (req.body.postDetails.postType === 1) {
        const updateTable = await youTubeModel.updateTable(req.body.userScopeId, accountId, teamId, req.body.postDetails);
      ...
      if (uploadedVideoCountPer24hrs.length >= 0 && config.get('youTube_upload_count_per_24hrs') <= uploadedVideoCountPer24hrs.length) {
        const remainingTme = await youTubeModel.remainingTimeForUpload((uploadedVideoCountPer24hrs[uploadedVideoCountPer24hrs.length - 1].updated_at));
      ...
      const response = await this.googleConnect.uploadYouTubeVideo(req.body.postDetails, socialAccountDetails.refresh_token);
      const updateTable = youTubeModel.updateTable(req.body.userScopeId, accountId, teamId, req.body.postDetails);
  ...
  async getYouTubePublishedDetails(req, res) {
    try {
      const {
        teamId, accountId, postType, pageId,
      } = req.query;
      ...
      const postDetails = await youTubeModel.postDetails(req.body.userScopeId, accountId, teamId, postType, pageId);
  ...
  async deleteYouTubePublishedDetails(req, res) {
    try {
      ...
      const result = await youTubeModel.getPostDetailsByIdWithPostType(req.body.userScopeId, req.query.postId);

      if (!result) return ErrorResponse(res, 'No record found');
      const response = await youTubeModel.deletePostDetailsById(req.body.userScopeId, req.query.postId, result.mongo_id);
  ...
  async getPublishedDetailsById(req, res) {
    try {
      ...
      if (!result) return ErrorResponse(res, 'No record found');
      const response = await youTubeModel.getPostDetailsByIdWithMongo(req.body.userScopeId, req.query.postId);
  ...
  async editPublishedDetails(req, res) {
    try {
      ...
      if (result.upload_type == 1) {
        if (req.body.postDetails.postType === 1) {
          const updateTable = await youTubeModel.updateDraftTable(req.body.userScopeId, accountId, teamId, req.body.postDetails, result.mongo_id, req.query.postId);
        ...
        if (req.body.postDetails.postType === 0) {
          ...
          const response = await this.googleConnect.uploadYouTubeVideo(req.body.postDetails, socialAccountDetails.dataValues.refresh_token);
          const updateTable = await youTubeModel.updateTable(req.body.userScopeId, accountId, teamId, req.body.postDetails);
          const removeData = await youTubeModel.removeData(result.mongo_id, req.query.postId);
