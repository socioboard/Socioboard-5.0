...
class YouTubeUploadValidator {
  ...
  validateUploadVideo(data) {
    const JoiSchema = Joi.object({
      ...
      postDetails: Joi.object({
        ...
        postType: Joi.number().min(0).max(1).required()
          .messages({ 'any.required': 'Post type required to upload', 'number.min': 'Post type should be 0 or 1', 'number.max': 'Post type should be 0 or 1' }),
  ...
  validateUploadVideoDetails(data) {
    const JoiSchema = Joi.object({
      ...
      postType: Joi.number().required().min(0).max(2)
        .messages({ 'any.required': 'Post type id required', 'number.min': 'Post type should be 0, 1 or 2', 'number.max': 'Post type should be 0, 1 or 2' }),
    }).options({ abortEarly: false });
