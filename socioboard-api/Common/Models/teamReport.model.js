...
class TeamReportLibs {
  ...
  async getTeamSchedulerStats(
    ...
    let {since, untill} = await this.getFilteredPeriod(
      filterPeriod,
      sinces,
      untils
    );
    let publishPostCount = await publishedPostsModel.getTeamSchedulerStats(
      socialAccounts,
      parseInt(teamId),
      since,
      untill
    );
    let scheduledCount = await scheduledPostsModel.getTeamSchedulerStats(
      parseInt(teamId),
      since,
      untill
    );
