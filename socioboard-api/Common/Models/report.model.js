...
class ReportModel {
  ...
  async getTodayPostedCount(userId, teamId, accountId) {
    try {
      let checkTeamValisForUser =
        await this.userTeamAccountLibs.isTeamAccountValidForUser(
          userId,
          teamId,
          accountId
        );
  ...
  async getXDayPublishCount(userId, dayCount) {
    try {
      ...
      if (!socialAcc)
        throw new Error(
          'Sorry, You dont have any social profiles to fetch reports'
        );
      else {
        logger.info(`Total account count : ${socialAcc.length}`);
        let publishedPostObject = new PublishedPost();
        let list = await socialAcc.map(async account => {
          let count = await publishedPostObject.getXdaysPostsCount(
            account.account_id,
            dayCount
          );
          var accountDetails = {
  ...
  async getAccountwisePublishCount(userId) {
    try {
      ...
      if (!socialAcc)
        ...
        let publishedPostObject = new PublishedPost();
        let list = await socialAcc.map(async account => {
          let count = await publishedPostObject.getAccountPublishCount(
            account.account_id
          );
          var accountDetails = {
            accountId: account.account_id,
  ...
  async getSchedulePublishedReport(scheduleId, userId, pageId) {
    try {
      let publishedPostObject = new PublishedPost();
      let result = await scheduleDetails.findOne({
        where: {schedule_id: scheduleId, user_id: userId},
      });
      if (!result) {
        throw new Error(
          'Sorry, Cant able to get schedule report due following reason. 1.Invalid user Id, 2.Invalid schedule Id'
        );
      } else {
        let publishedDetails =
          await publishedPostObject.getSchedulePublishedReport(
            result.mongo_schedule_id,
            (pageId - 1) * config.get('perPageLimit'),
  ...
  async getAccountPublishedReport(userId, accountId, teamId, pageId) {
    try {
      if (accountId != 0) {
        let checkTeamValid = await this.userTeamAccountLibs.isTeamValidForUser(
          userId,
          teamId
        );
        let accountValidTeam =
          await this.userTeamAccountLibs.isAccountValidForTeam(
            teamId,
            accountId
          );
        let publishedPostObject = new PublishedPost();
        let publishedDetails =
          await publishedPostObject.getAccountPublishedReport(
            accountId,
            teamId,
            (pageId - 1) * config.get('perPageLimit'),
            config.get('perPageLimit')
        ...
        return this.isTeamAccountValidForUser(userId, teamId, accountId)
          ...
          .then(socialAccount => {
            if (socialAccount == null)
              throw new Error(
                "No profile found or account isn't twitter profile."
              );
            else {
              var offset = (pageId - 1) * config.get('perPageLimit');
              var twitterMongoMessageModelObject =
                new TwitterMongoMessageModel();
              return twitterMongoMessageModelObject.getSocialAccountPosts(
        ...
        return socialAccount
          ...
          .then(socialAccount => {
            if (socialAccount == null)
              ...
              var twitterMongoMessageModelObject =
                new TwitterMongoMessageModel();
              return twitterMongoMessageModelObject.getMessageBetweenTwoUsers(
                socialAccount.social_id,
                receiverId,
                offset,
                config.get('perPageLimit')
        ...
        return socialAccount
          ...
          .then(socialAccount => {
            if (socialAccount == null)
              ...
              var twitterMongoMessageModelObject =
                new TwitterMongoMessageModel();
              return twitterMongoMessageModelObject.getPreviouslyMessagedUsers(
                socialAccount.social_id,
