...
class ReportMailModel {
  ...
  async getTeamSchedulerStats(teamId, userId, frequency) {
    ...
    if (validTeam) {
      ...
      const report = await teamReport.getTeamSchedulerStats(
        teamSocialAccount,
        frequency,
        teamId
      );
  ...
  async getTwitterReport(accountId, userScopeId, filterPeriod) {
    ...
    if (socialAccDetails) {
      ...
      const twitterInsight = await twitterInsights.getInsightsDaywise(
        accountId,
        dates.since,
        dates.untill
      );
      ...
      const stats = await twitterInsights.getInsightsStats(
        accountId,
        dates.since,
        dates.untill
      );
  ...
  async fbpageReport(accountId, userScopeId, filterPeriod) {
    ...
    if (getSocialAccDetails) {
      const dates = await teamReport.getFilteredPeriod(filterPeriod);
      const response = await this.facebookHelper.fbPageInsights(
