    ...
    async getFacebookPages(code) {
        ...
        return new Promise((resolve, reject) => {
            if (!code) {
                ...
                return this.facebookHelper.getOwnFacebookPages(code, config.get('profile_page_redirect_url'))
                    .then((pageDetails) => {
                        if (!pageDetails) {
    ...
    async getOwnFacebookGroups(code) {
        ...
        return new Promise((resolve, reject) => {
            if (!code) {
                ...
                this.facebookHelper.getOwnFacebookGroups(code, config.get('facebook_api.fbprofile_add_redirect_url'))
                    .then((groupInfo) => {
    ...
    return new Promise((resolve, reject) => {
      if (!code) {
        ...
        return this.googleHelper
          ...
          .then(response => {
            let channelDetails = response.parsedBody;
            let tokens = response.tokens;
            if (channelDetails?.items) {
              let etag = channelDetails.etag;
          ...
          .then(matchedChannels => {
            if (matchedChannels) {
              matchedChannels.map(channel => {
                let matchedChannel = channels.find(
