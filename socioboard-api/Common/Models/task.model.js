...
class TaskModel {
    ...
    return new Promise((resolve, reject) => {
      if (!userId || !teamId || !pageId) {
        ...
        return this.isTeamValidForUser(userId, teamId)
          .then(() => {
            return taskModels.getTeamPublishTaskLists(
              userId,
              teamId,
              skip,
          ...
          .then(() => {
            if (normalPostIds.length > 0) {
              ...
              return adminpostlists
                .getPostsById(normalPostIds)
                .then(result => {
          ...
          .then(() => {
            resolve({
              ...
              data: {
                count: tasks.length,
                tasks: tasks,
                schedule_details: scheduleInfos,
  ...
  updateTaskStatus(userId, taskId, status, teamId) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!userId || !taskId || !status || !teamId)
        ...
        var taskResult = {};
        return taskModels
          .updatePublishTaskStatus(userId, taskId, teamId, status)
          .then(result => {
            if (!result)
              throw new Error(
              ...
              var savedId =
                taskResult.type == 2
                  ? taskResult.normalPostId
                  : taskResult.schedulePostId;
              if (taskResult.type == 2) {
                var adminpostlists = new AdminApprovalPostModels();
                return adminpostlists.updateAdminResponse(savedId, status);
              } else if (taskResult.type == 3) {
                return scheduleDetails
                  .findOne({where: {schedule_id: savedId}})
                  .then(result => {
                    if (!result)
                      throw new Error(
                        'Invalid id to fetch the schedule details'
                      );
                    else {
                      var schedulePost = new SchedulePostModels();
                      return schedulePost.updateAdminResponse(
                        result.mongo_schedule_id,
                        status
                      );
                    }
                  })
          ...
          .then(() => {
            if (status != 'approved')
              ...
              } else throw new Error('Invalid task type to perform an action');
            }
          })
          .then(result => {
  ...
  afterPublishNowApproved(postSavedId, teamId) {
    return new Promise((resolve, reject) => {
      if (!postSavedId || !teamId) {
        ...
        var adminpostlists = new AdminApprovalPostModels();
        return adminpostlists
          .getUnpublishedPostById(postSavedId)
          .then(result => {
            if (!result) {
              ...
              if (!result.description) {
                ...
                return publishLibs.startPublish(
                  postDetails,
                  teamId,
                  result.accountIds
                );
      ...
      if (!postSavedId) {
        ...
        return scheduleDetails
          ...
          .then(result => {
            if (!result) {
              throw new Error(
                'Sorry, Schedule details arent in ready state, may be in pause or done state!'
              );
            } else {
              logger.info(`Fetch post details : ${JSON.stringify(result)}`);

              var schedulePostModels = new SchedulePostModels();
              return schedulePostModels
                .getScheduleDetails(result.mongo_schedule_id)
                .then(scheduleMongoDetails => {
                  logger.info(
                    `Schedule details : ${JSON.stringify(scheduleMongoDetails)}`
                  );
                  ...
                  if (!scheduleMongoDetails)
                    ...
                    var scheduleTimings = [];
                    scheduleTimings.push(
                      scheduleMongoDetails.normalScheduleDate
                    );
                    return this.getRunningDayAndTodayTimings(scheduleTimings)
                      .then(result => {
                        logger.info(
                          `Timings : ${JSON.stringify(result.todaysTiming)}`
                        );
                        return result.todaysTiming;
                      })
                      .catch(error => {
                        throw new Error(
                          'Cant able to get the schedule timings!'
                        );
                      });
                  } else if (result.schedule_type == 1) {
                    return this.getRunningDayAndTodayTimings(
                      scheduleMongoDetails.daywiseScheduleTimer
                    )
                      .then(result => {
                        logger.info(
                          `Timings : ${JSON.stringify(result.todaysTiming)}`
                    ...
                    throw new Error(
                      `Invalid schedule status and status is ${scheduleMongoDetails.schedule_status}!`
                    );
                  }
                });
            }
          ...
          .then(todaysTiming => {
            if (todaysTiming.length > 0) {
              ...
              return scheduleBase
                .makeSchedule(postSavedId, todaysTiming)
                .then(response => {
                  resolve({
                    code: 200,
                    status: 'success',
                ...
                .catch(response => {
                  resolve({
                    code: 200,
                    status: 'success',
                    expiredTimings: response.expiredTimes,
                  });
                });
            } else {
              resolve({code: 200, status: 'success'});
          ...
          if (runningDay.dayId == undefined || runningDay.dayId == null) {
            ...
            if (runningDay.dayId == currentDayOfWeek) {
              runningDay.timings.forEach(time => {
                logger.info(
                  `Schedule timing hours : ${moment(time).utc().hour()}`
                );
  ...
  setupTwitterInsightsCrons() {
    schedule.scheduleJob('00 22 * * *', () => {
      logger.info(
        `Cron started for updating all twitter account insight's which is not locked for atleast a team, started time ${moment()}`
      );
      new Promise((resolve, reject) => {
        ...
        return socialAccounts
          ...
          .then(result => {
            if (result.length > 0) {
              return Promise.all(
                result.map(socialAccount => {
                  twtAccount = socialAccount;
                  return teamSocialAccount
                    .findOne({
                      where: {
                    ...
                    .then(runnigAccount => {
                      logger.info(
                        `RunnigAccount ${
                          (runnigAccount.dataValues.account_id,
                      ...
                      if (runnigAccount) {
                        ...
                        return this.twitterConnect
                          ...
                          .then(() => {
                            logger.info(
                              `Updated twitter insight for ${socialAccount.user_name}`
                            );
                            var insightObject = {
                              followerCount: updatedTwitterStats.follower_count,
                              followingCount:
                                updatedTwitterStats.following_count,
                              favouritesCount:
                                updatedTwitterStats.favorite_count,
                            ...
                            var twtMongomodelObject =
                              new TwitterInsightMongoModel();
                            return twtMongomodelObject.addInsights(
                              accountId,
                              insightObject
                            );
                          })
