...
class YouTubeModel {
  ...
  async updateTable(user_id, account_id, team_id, postDetails) {
    try {
      const postInfo = await this.postInfo(postDetails, user_id, team_id, account_id);
  ...
  async deletePostDetailsById(user_id, id, mongo_id) {
    ...
    const deleteData = await YouTubeUploadDetails.destroy({ where: { user_id, id } });
  ...
  async removeData(mongo_id, id) {
    ...
    const removeData = await YouTubeUploadPosts.findByIdAndRemove({ _id: String(mongo_id) });
