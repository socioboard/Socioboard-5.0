                  ...
                  .then(teamDetails => {
                    if (!teamDetails) {
                      ...
                      if (teamDetails.is_account_locked == 1)
                        ...
                        var clonedPostDetails = JSON.parse(
                        ...
                        switch (SocialAccount.account_type) {
                          ...
                          case 11:
                            return this.validatePinterestPosts(
                              clonedPostDetails
                            )
                              .then(() => {
                                return this.publishOnPinterest(
                                  clonedPostDetails,
                                  SocialAccount.account_id,
                                  SocialAccount.access_token,
                                  SocialAccount.refresh_token,
    ...
    if (postDetails.postType == 'Video') {
      return this.getThumbnailOfMedia(postDetails.mediaPath)
        .then(mediaInfos => {
          if (accountType == 6) {
            ...
            return this.linkedInConnect.publishPost(
              'person',
              postDetails,
              accessToken,
              userName
      ...
      if (accountType == 6) {
        return this.linkedInConnect
          .publishPost('person', postDetails, accessToken, userName)
          .then(response => {
        ...
        .then(postCount => {
          ...
          if (postCount < config.get('twitter_api.maximum_post_per_day')) {
            return this.twitterConnect
              ...
              .then(() => {
                if (config.get('notifications.status') == 'on')
    ...
    return new Promise((resolve, reject) => {
      ...
      publishedPostObject.getTodayPostsCount(accountId).then(postCount => {
        if (postCount < config.get('pinterest.maximum_post_per_day')) {
          var boards = lodash.filter(postDetails.boardDetails, {
            accountId: accountId,
          });
