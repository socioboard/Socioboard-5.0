...
import TeamReport from './teamReport.model.js';
...
import TwitterInsights from '../Mongoose/models/twitterInsights.js';
...
import SchedulePostModel from '../Mongoose/models/scheduleposts.js';
...
class ReportMailModel {
  ...
  async isUserBelongsToTeam(userId, teamId) {
    let response = await userTeamJoinTable.findOne({
      where: {
        team_id: teamId,
        user_id: userId,
        left_from_team: 0,
  ...
  async getTeamSchedulerStats(teamId, userId, frequency) {
    let validTeam = await this.isUserBelongsToTeam(userId, teamId);
    let teamSocialAccount;
    if (validTeam) {
      let teamDetails = await this.getTeamDetails(teamId);
      teamSocialAccount = await teamReport.teamSocialAccount(teamId);
      let report = await teamReport.getTeamSchedulerStats(
        teamSocialAccount,
        frequency,
        teamId
      );
  ...
  async getTwitterReport(accountId, userScopeId, filterPeriod) {
    let socialAccDetails = await this.getSocialAccount(4, accountId);
    if (socialAccDetails) {
      let dates = await teamReport.getFilteredPeriod(filterPeriod);
      let twitterInsight = await twitterInsights.getInsightsDaywise(
        accountId,
        dates.since,
        dates.untill
      );
      let data = await this.formatDate(
        twitterInsight,
        dates.since,
        dates.untill
      );
      let stats = await twitterInsights.getInsightsStats(
        accountId,
        dates.since,
        dates.untill
      );
  ...
  async fbpageReport(accountId, userScopeId, filterPeriod) {
    let getSocialAccDetails = await this.getSocialAccount(2, accountId);
    if (getSocialAccDetails) {
      let dates = await teamReport.getFilteredPeriod(filterPeriod);
      let response = await this.facebookHelper.fbPageInsights(
        getSocialAccDetails.access_token,
        getSocialAccDetails.social_id,
        dates.since,
        dates.untill,
        filterPeriod
      );
      if (response.data.length < 1)
        throw new Error("Sorry, Account isn't active for specified time span");
      return response;
    }
  }
  ...
  async getYouTubeReport(accountId, userScopeId, filterPeriod) {
    let socialAccDetails = await this.getSocialAccount(9, accountId);
    if (socialAccDetails) {
      let dates = await teamReport.getFilteredPeriod(filterPeriod); //modification required
      let data = await this.googleHelper.youtubeInsights(
        socialAccDetails.refresh_token,
        encodeURIComponent(socialAccDetails.social_id),
        dates.since,
        dates.untill
      );
      let response = await this.parseYouTubeData(data);
  ...
  async parseYouTubeData(youTubeInsightData) {
    ...
    let data = await this.getTotalData(result);
    return data;
  }
  ...
  async formatDate(twitterInsights, since, untill) {
    ...
    let daywisesData = [];
    for (let count in dates) {
      let daywises = {
        date:
          dates[count].getFullYear() +
          '-' +
          ('0' + (dates[count].getMonth() + 1)).slice(-2) +
          '-' +
          ('0' + dates[count].getDate()).slice(-2),
        followerCount: 0,
        followingCount: 0,
        favouritesCount: 0,
        postsCount: 0,
    ...
    for (let d in daywisesData) {
      for (let c in twitterInsights) {
  ...
  async teamReportToPdf(details, reportTitle, teamName, frequency, teamId) {
    try {
      const filePath = paths.resolve(
        __dirname,
        `../../../media/autoReport/${reportTitle}_${teamName}_${frequency}_${teamId}_${
          details[0].date
        }_${details[details.length - 1].date}.pdf`
      );
      ...
      let pdfFileLogoUrl = 'https://i.imgur.com/eRkLsuQ.png';
      ...
      let logo = await pdfDoc.setLogo(pdfFileLogoUrl);
      ...
      logger.error(
        `-V3---email-----${err.message}---${err}'---${__filename}----`
      );
  ...
  async twitterReportToPdf(
    ...
    try {
      const filePath = paths.resolve(
        __dirname,
        `../../../media/autoReport/${reportTitle}_${user_name}_${frequency}_${account_id}_${
          details[0].date
        }_${details[details.length - 1].date}.pdf`
      );
      const pdfFileTableHeader = [
  ...
  async youtubeReportToPdf(
    ...
    try {
      const filePath = paths.resolve(
        ...
        `../../../media/autoReport/${reportTitle}_${user_name}_${frequency}_${account_id}_${
          details[0].day
        }_${details[details.length - 1].day}.pdf`
      );
      const pdfFileTableHeader = [
        {id: 'day', title: 'Date'},
  ...
  async teamReportToCsv(DayWiseData, reportTitle, teamName, frequency, teamId) {
    try {
      const fileName = `${reportTitle}_${teamName}_${frequency}_${teamId}_${
        DayWiseData[0].date
      }_${DayWiseData[DayWiseData.length - 1].date}.csv`;
  ...
  async twitterReportToCsv(
    ...
    try {
      const fileName = `${reportTitle}_${user_name}_${frequency}_${account_id}_${
        details[0].date
      }_${details[details.length - 1].date}.csv`;
  ...
  async youtubeReportToCsv(
    ...
    try {
      const fileName = `${reportTitle}_${user_name}_${frequency}_${account_id}_${
        Details[0].day
      }_${Details[Details.length - 1].day}.csv`;
      const {filePath, csvWriter} = this.getCvsWriter(fileName, [
        {id: 'day', title: 'Date'},
        {id: 'averageViewDuration', title: 'Average View Duration'},
        {id: 'likes', title: 'Total likes'},
  ...
  async addReport(report_title, report_type, frequency, content, user_id) {
    let res = await ReportMailDetails.create({
      report_title,
      report_type,
      frequency,
      content,
  ...
  async editReport(report_title, report_type, frequency, content, user_id, id) {
    let res = await ReportMailDetails.update(
      {
        report_title,
        report_type,
        frequency,
  ...
  async removeReport(id) {
    let res = await ReportMailDetails.destroy({where: {id}});
    return res;
  }
  ...
  async getReportById(id) {
    let res = await ReportMailDetails.findOne({where: {id}});
    return res;
  }
  ...
  async getReports(user_id, pageId) {
    let res = await ReportMailDetails.findAll({
      where: {user_id},
  ...
  async getDirectTotalPost(ownerId, teams) {
    ...
    let publishPost = new PublishedPost();
    if (teams.length != 0) {
    ...
    let res = await publishPost.getTotalTeamSchedulerStats(TeamId);
    let scheduleCount = await SchedulePostModel.countDocuments({ownerId});
    let teamDetails = await teamSocialAccountJoinTable.findAll({
    ...
    let result = await socialAccount.findAll({
      where: {account_id},
      attributes: [
        'account_type',
    ...
    return {
      totalPost: res[0]?.postCount ?? 0 + scheduleCount,
      scheduleCount,
      publishedCount: res[0]?.postCount ?? 0,
      postFailed: res[0]?.postFailed ?? 0,
      youTubeCount,
      twitterCount,
      fbCount,
      instagramCount,
  ...
  async getReportDate(filterPeriod) {
    let {since, untill} = await teamReport.getFilteredPeriod(filterPeriod);
    return `${moment(since).format('YYYY-MM-DD')} to ${moment(untill).format(
      'YYYY-MM-DD'
    )}`;
  }
}
