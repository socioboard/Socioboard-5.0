...
class ReportMailModel {
    ...
    async getTeamSchedulerStats(teamId, userId, frequency) {
        ...
        if (validTeam) {
            ...
            let report = await teamReport.getTeamSchedulerStats(teamSocialAccount, frequency, teamId)
    ...
    async getTwitterReport(accountId, userScopeId, filterPeriod) {
        ...
        if (socialAccDetails) {
            ...
            let twitterInsight = await twitterInsights.getInsightsDaywise(accountId, dates.since, dates.untill)
            let data = await this.formatDate(twitterInsight, dates.since, dates.untill)
            let stats = await twitterInsights.getInsightsStats(accountId, dates.since, dates.untill)
    ...
    async fbpageReport(accountId, userScopeId, filterPeriod) {
        ...
        if (getSocialAccDetails) {
            let dates = await teamReport.getFilteredPeriod(filterPeriod)
            let response = await this.facebookHelper.fbPageInsights(getSocialAccDetails.access_token, getSocialAccDetails.social_id, dates.since, dates.untill, filterPeriod)
    ...
    async getYouTubeReport(accountId, userScopeId, filterPeriod) {
        ...
        if (socialAccDetails) {
            ...
            let data = await this.googleHelper.youtubeInsights(socialAccDetails.refresh_token, encodeURIComponent(socialAccDetails.social_id), dates.since, dates.untill)
    ...
    */
    ...
    async addReport(report_title, report_type, frequency, content, user_id) {
        let res = await ReportMailDetails.create({
            report_title, report_type, frequency, content, user_id, created_at: moment.utc(),
    ...
    */
    ...
    async getDirectTotalPost(ownerId, teams) {
        ...
        else TeamId = await teamInfo.findAll({
            where: { team_admin_id: ownerId },
            attributes: ['team_id'], raw: true
        })
        ...
        let youTubeCount = 0, twitterCount = 0, fbCount = 0, instagramCount = 0, linkedInCount = 0;
        result.map(x => {
        ...
        return { totalPost: (res[0]?.postCount ?? 0 + scheduleCount), scheduleCount, publishedCount: res[0]?.postCount ?? 0, postFailed: res[0]?.postFailed ?? 0, youTubeCount, twitterCount, fbCount, instagramCount, linkedInCount }
