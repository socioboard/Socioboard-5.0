...
import SchedulePost from '../Mongoose/models/scheduleposts.js'
import ScheduleBase from '../../Publish/core/schedule/schedulebase.js';
...
class ScheduleLibs extends ScheduleBase {
                ...
                return this.verifyAccounts(postInfo.postingSocialIds)
                    ...
                    .then((teamUserResponse) => {
                        if (!teamUserResponse)
                            ...
                            postInfo.adminResponseStatus = postInfo.scheduleStatus == 5 ? "draft" : usersTeamPermission == 1 || 2 ? "fullrights" : "pending";
                    ...
                    .then((response) => {
                        ...
                        if (postInfo.scheduleStatus == 5) {
                            ...
                            if (todaysTiming.length > 0) {
                                return this.makeSchedule(scheduledId, todaysTiming)
                                    ...
                                    .catch((expiredTimings) => {
                                        resolve({ message: createdResponse, expiredTimings: expiredTimings });
                                    });
                            }
                            else
                            ...
                            return this.createScheduleTask(postInfo.teamId, userId, userName, response.schedule_id)
                                .then(() => {
                                    resolve({ message: "Submitted a request to admin for schedule a post!" });
                                })
        ...
        return new Promise((resolve, reject) => {
            if (!userId || !userName || !postInfo) {
                ...
                return scheduleDetails.findOne({
                    ...
                    .then(() => {
                        postInfo.ownerId = userId;
                        postInfo.adminResponseStatus = scheduleIdDetails.schedule_status == 1 || 2 ? "fullrights" : "pending";
    ...
    async editDraftSchedule(userId, userName, teamId, scheduleId, postInfo) {
        let id = await this.getScheduleMongoId(userId, scheduleId)
        let schedulePost = new SchedulePost();
        let update = await schedulePost.insertManyPosts([postInfo], id)
        let res = await schedulePost.getPostsById(id)
        return id;
    }
    ...
    getSchedulePostById(userId, mongoIds) {
        return new Promise((resolve, reject) => {
            return SchedulePost.find({ _id: { $in: mongoIds } }, '-_id', { lean: true })
                .then((mongoValues) => {
                    resolve(mongoValues);
                })
                .catch((error) => {
    ...
    async getScheduleId(userId, mongo_schedule_id, response) {
        let scheduleData = await scheduleDetails.findOne({
            where: {
                [Operator.and]: [{
                    user_id: userId
                }, {
            ...
            if (!userId || !scheduleStatus || !Boolean(Number(pageId)) || scheduleStatus > 7 || scheduleStatus < 1) {
                ...
                return scheduleDetails.findAll({
                    ...
                    if (response) {
                        ...
                        return SchedulePost.find({ _id: { $in: mongoIds } }).sort({ createdDate: -1 })
                    } else
                        return [];
                })
                    .then((mongoValues) => {
     ...
     */
    async getScheduleDetailsForCalenderView(user_id, schedule_status, schedule_type) {
        let query = { user_id };
        if (schedule_status) query.schedule_status = schedule_status
        if (schedule_type) query.schedule_type = schedule_type
        let mongoIds = [];
        let response = await scheduleDetails.findAll({
            where: query,
            order: [
                ['created_date', 'DESC'],
            ],
        ...
        if (response) {
            ...
            return await SchedulePost.find({ _id: { $in: mongoIds } }).sort({ createdDate: -1 });
        } else return [];
    }
                ...
                return this.getActiveScheduleCount(userId)
                    ...
                    .then(() => {
                        if (userPermission == 0 && newScheduleStatus == 1)
                            resolve({ message: 'Schedule status has been updated successfully. Admin approvals is in pending!' });
                        else
                            resolve({ message: 'Schedule status has been updated successfully!' });
                    })
                    .catch((error) => {
                        reject(error);
                    });
    ...
    startSchedulerCron() {
        return new Promise((resolve, reject) => {
            return this.setupCrons()
                .then(() => {
                    resolve({ message: "Scheduler cron setup has been successfully done!" });
                })
                .catch((error) => {
                    reject(error);
                });
