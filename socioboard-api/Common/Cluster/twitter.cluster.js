...
Twitter.prototype.accessToken = function (token, secret, verifier) {
  return new Promise((resolve, reject) => {
    this.twitterObj.getAccessToken(token, secret, verifier, (error, accessToken, accessSecret) => {
      // Checking whether it sent error in callback or not
      if (error) reject(error);
      else {
        const response = {
...
Twitter.prototype.verifyCredentials = function (accessToken, accessSecret) {
  return new Promise((resolve, reject) => {
    this.twitterObj.verifyCredentials(accessToken, accessSecret, (error, user) => {
      // Checking whether it sent error in callback or not
      if (error) {
        reject(error);
      } else {
              ...
              tweet.extended_entities.media.forEach((media) => {
                ...
                if (media.type == 'photo') {
                  ...
                  media.video_info.variants.forEach((variants) => {
                    if (variants.content_type == 'video/mp4') if (variants.bitrate == 832000) mediaUrl = variants.url;
                  });
                } else if (media.type == 'animated_gif') {
                  media.video_info.variants.forEach((variants) => {
                    if (variants.content_type == 'video/mp4') mediaUrl = variants.url;
                  });
                }
  ...
  // Fetching timeline tweets from twitter
    this.getTimeLineTweets(accessToken, refreshToken, userName)
      ...
      // Formating the response of timeline tweets
        this.parseTweetDetails(timelineTweets, socialId, this.twitter_api.app_name, this.twitter_api.version))
      .then((tweets) => {
        // Sending response
        resolve(tweets);
      })
...
Twitter.prototype.getFollowersList = function (accessToken, accessTokenSecret, cursorValue) {
  return new Promise((resolve, reject) => {
    ...
    if (!accessToken || !accessTokenSecret) {
      ...
      const twitterClient = this.getTwitterClient(accessToken, accessTokenSecret);
      const queryString = { count: 200, include_user_entities: false, skip_status: true };
  ...
  return new Promise((resolve, reject) => {
    ...
    if (!messageDetails || !accessToken || !accessTokenSecret) {
      ...
      if (messageDetails.messageType == 'Text') {
        const message = { event: { type: 'message_create', message_create: { target: { recipient_id: messageDetails.recipientId }, message_data: { text: messageDetails.text } } } };
  ...
  return new Promise((resolve, reject) => {
    ...
    if (!postDetails || !accessToken || !accessTokenSecret) {
      ...
      if (postDetails.postType == 'Text') {
        twitterClient.post('statuses/update', { status: postDetails.message }, (error, tweet, response) => {
          // Checking whether it sent error in callback or not
          if (error) {
        ...
        return Promise.all(postDetails.mediaPath.map((media) => {
          ...
          .then(() => {
            ...
            if (postDetails.link && postDetails.link != '') message = `${postDetails.message} \r\n Link: ${postDetails.link}`;
            ...
            return twitterClient.post('statuses/update', status, (error, tweet, response) => {
              // Checking whether it sent error in callback or not
              if (error) {
                reject({ code: 400, status: 'failed', message: error });
              } else {
                // Sending response
                resolve({
                  code: 200, status: 'success', message: tweet, failedMediaPath: erroredImages,
                });
              }
            });
          })
...
Twitter.prototype.getGeoLocationDetails = function (keyword, location, accessToken, accessTokenSecret, callback) {
  ...
  twitterClient.get('search/tweets', { q: keyword, geocode: location }, (error, location, response) => {
    // Checking whether it sent error in callback or not
    if (error) {
      callback({ code: 400, status: 'failed', message: error });
    } else {
...
Twitter.prototype.likeTwitterPost = function (tweetId, accessToken, accessTokenSecret) {
  return new Promise((resolve, reject) => {
    ...
    if (!tweetId || !accessToken || !accessTokenSecret) {
      ...
      twitterClient.post('favorites/create', { id: tweetId }, (error, tweet, response) => {
        // Checking whether it sent error in callback or not
        if (error) {
          reject(error[0]);
        } else {
...
Twitter.prototype.unlikeTwitterPost = function (tweetId, accessToken, accessTokenSecret) {
  return new Promise((resolve, reject) => {
    ...
    if (!tweetId || !accessToken || !accessTokenSecret) {
      ...
      twitterClient.post('favorites/destroy', { id: tweetId }, (error, tweet, response) => {
        // Checking whether it sent error in callback or not
        if (error) {
          reject(error);
        } else {
...
Twitter.prototype.getAllTweets = function (accessToken, accessTokenSecret, screenName, lastTweetId) {
  return new Promise(async (resolve, reject) => {
    if (!accessToken || !accessTokenSecret || !screenName) {
      ...
      const parameters = { screen_name: screenName, count: '200', tweet_mode: 'extended' };
...
Twitter.prototype.unretweetPost = function (tweetId, accessToken, accessTokenSecret, callback) {
  return new Promise((resolve, reject) => {
    if (!tweetId || !accessToken || !accessTokenSecret) {
      ...
      twitterClient.post(`statuses/unretweet/${tweetId}`, {}, (error, tweet, response) => {
        if (error) {
          reject(error[0]);
        } else {
          resolve(response);
...
Twitter.prototype.fetchTweetsKeywords = function (twitterdata, keywords, max_id, since_id) {
  return new Promise((resolve, reject) => {
    if (!twitterdata.access_token || !twitterdata.access_secret) {
      ...
      const twitterClient = this.getTwitterClient(twitterdata.access_token, twitterdata.access_secret);
...
Twitter.prototype.followTwitterId = function (accessToken, accessTokenSecret, screen_name) {
  return new Promise((resolve, reject) => {
    ...
    if (!accessToken || !accessTokenSecret) {
      ...
      twitterClient.post('friendships/create', queryString, (error, tweet, response) => {
        // Checking whether it sent error in callback or not
        if (error) {
          reject(error[0]);
        } else {
