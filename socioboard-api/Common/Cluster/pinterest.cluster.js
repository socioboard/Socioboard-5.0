...
Pinterest.prototype.addPinterestProfile = function addFacebook(
  network,
  teamId,
  code
) {
  ...
  return new Promise((resolve, reject) => {
    if (!code) {
      ...
      // Calling function of getting accessToken by giving code as input
      return this.getAppAccessToken(code)
        ...
        .catch(error => {
          if (error.message && error.message.includes('Authorization failed.'))
            ...
            reject(
              new Error('You have exceeded your rate limit. Try again later.')
            );
          else reject(error);
        });
    }
...
Pinterest.prototype.userProfileInfo = function (accessToken) {
  return new Promise((resolve, reject) => {
    ...
    return requestPromise(
      `https://api.pinterest.com/v1/users/me/?access_token=${accessToken}&fields=first_name%2Cid%2Clast_name%2Curl%2Cusername%2Cimage%2Ccounts%2Cbio%2Caccount_type`
    )
      .then(body => {
...
Pinterest.prototype.userDetails = function (UserName, accessToken) {
  return new Promise((resolve, reject) => {
    if (!UserName || !accessToken) {
      ...
      request.get(
        `https://api.pinterest.com/v1/users/${UserName}/?access_token=${accessToken}&fields=first_name%2Cid%2Clast_name%2Curl%2Ccounts%2Cbio%2Cimage`,
