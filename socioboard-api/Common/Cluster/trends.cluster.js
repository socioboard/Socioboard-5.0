...
class Trends {
  fetchGiphy(giphy, keyword, pageId, rating, type) {
    return new Promise((resolve, reject) => {
      if (!giphy || !keyword || !pageId || !rating || !type) {
        ...
        get(
          {
            url: `https://api.giphy.com/v1/${type}/search?q=${encodeURIComponent(
              keyword
            )}&api_key=${giphy.api_key}&offset=${offset}&limit=${
              giphy.count
            }&rating=${rating}`,
          },
          function (error, response, body) {
            ...
            if (error) {
              return giphyModelObject
                .getPreviousPost(keyword, offset, giphy.count)
                .then(giphyDetails => {
                ...
                parsedBody.data.map(element => {
                  try {
                    ...
                    var details = {
                      ...
                      postSourceUrl: element.source_post_url
                        ? encodeURI(element.source_post_url)
                        : '',
                      mediaUrl: element.images.preview_gif.url
                        ? encodeURI(element.images.preview_gif.url)
                        : '',
      ...
      if (!imgurApi || !keyword || !page_id || !sortBy) {
        ...
        get(
          ...
          (error, response, body) => {
            ...
            if (error) {
              imgurModelObject
                .getPreviousPost(keyword, offset, imgurApi.count)
                .then(imgurDetails => {
              ...
              parsedBody.data.forEach(element => {
                let media_type = element.type
                  ? element.type
                  : element.images[0].type;
                var mediaUrls = [];
                if (element.images) {
                ...
                var details = {
                  ...
                  publishedDate: moment(Number(element.datetime) * 1000).format(
                    'M/D/YYYY H:mm'
                  ),
              ...
              return imgurModelObject
                .insertManyPosts(imgurDetails)
                .then(() => {
                  imgurResponse.imgurDetails = imgurDetails;
                  resolve(imgurResponse);
                })
      ...
      if (!giphy || !type || !pageId || !rating) {
        ...
        get(
          ...
          function (error, response, body) {
            ...
            if (error) {
              console.log(
                `Cant able to fetch the giphy posts, ${error.message}`
              );
              return giphyModelObject
                .getPreviousPost(keyword, offset, giphy.count)
                .then(giphyDetails => {
              ...
              return Promise.all(
                parsedBody.data.map(element => {
                  try {
                    ...
                    var details = {
                      giphyId: element.id,
                      sourceUrl: element.source_tld
                        ? encodeURI(element.source_tld)
                        : '',
                      title: element.title,
                      description: element.slug,
                      publisherName: element.username,
        ...
        newsapi.v2
          ...
          .then(response => {
            ...
            if (parsedBody.status == 'ok') {
              ...
              return newsApiMongoObject
                .insertManyPosts(newsApiDetails)
                .then(() => {
          ...
          .catch(error => {
            return newsApiMongoObject
              .getPreviousPost(keyword, offset, news_api.count)
              .then(newsApiDetails => {
                newsApiResponse.newsApiDetails = newsApiDetails;
                resolve(newsApiResponse);
              })
        ...
        get(
          {
            url: `https://pixabay.com/api/?key=${
              ...
              keyword
            )}&image_type=photo&pretty=true&page=${pageId}&category=${category}&order=${sortBy}`,
          },
          function (error, response, body) {
            var batchId = String(moment().unix());
            var pixabayResponse = {
            ...
            if (error) {
              return pixabayModelObject
                .getPreviousPost(keyword, offset, pixabay.count)
                .then(pixabayDetails => {
              ...
              return pixabayModelObject
                .insertManyPosts(pixabayDetails)
                .then(() => {
                  pixabayResponse.pixabayDetails = pixabayDetails;
                  resolve(pixabayResponse);
                })
      ...
      if (!flickr || !keyword || !pageId || !sortBy) {
        ...
        get(
          ...
          function (error, response, body) {
            ...
            if (error) {
              return flickrModelObject
                .getPreviousPost(keyword, offset, flickr.count)
                .then(flickrResponse => {
      ...
      if (!dailymotion || !page_id) {
        ...
        get(
          ...
          (error, response, body) => {
            ...
            if (error) {
              return dailymotionMongoObject
                .getPreviousPost(offset, dailymotion.count)
              ...
              return dailymotionMongoObject
                .insertManyPosts(dailyMotionDetails)
                .then(() => {
                  dailymotionResponse.dailymotionDetails = dailyMotionDetails;
                  resolve(dailymotionResponse);
                })
          ...
          return parse(parseData)
            ...
            .catch(error => {
              if (error.message.includes('ESOCKETTIMEDOUT')) {
                reject(
                  new Error(
                    'Sorry, Currently cant able to fetch the feeds details.'
                  )
                );
              }
              reject(error);
  ...
  fetchYoutube(youtube, pageId, keyword, sortby) {
    return new Promise((resolve, reject) => {
      ...
      get(
        {
          url: `https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=${offset}&order=${sortby}&q=${encodeURI(
            keyword
          )}&key=${youtube.api_key}`,
        },
        function (error, response, body) {
          ...
          if (error) {
            return youtubeModelObject
              .getPreviousPost(keyword, offset, youtube.count)
              .then(youtubeDetails => {
                if (youtubeDetails.length > 0)
                  youtubeResponse.youtubeDetails = youtubeDetails;
                else
                  throw new Error(
                    `${JSON.stringify(parsedBody.error.message)}`
                  );
            ...
            return youtubeModelObject
              .insertManyPosts(youtubeDetails)
              .then(() => {
                youtubeResponse.youtubeDetails = youtubeDetails;
                resolve(youtubeResponse);
              })
        ...
        get(options, (error, response, body) => {
          if (error) {
            ...
            trendsSplitUp.forEach(data => {
              var details = {
                title: this.getBetween(
                  data,
                  'data-trend-name=\\"',
                  '\\"'
                ).trim(),
                tweetCount: this.getBetween(
                  data,
                  'js-ellipsis\\">\\n',
                  'Tweets'
                ).trim(),
        ...
        get(
          ...
          (error, response, body) => {
            if (error) {
              ...
              trendsSplitUp.forEach(data => {
                var tweetUrl = this.getBetween(
                  data,
                  'data-permalink-path=\\"',
                  '\\"'
                );
                var userScreenName = this.getBetween(
                  data,
                  'data-screen-name=\\"',
                  '\\"'
                );
                ...
                var tweetedTimeStamp = this.getBetween(
                  data,
                  'data-time=\\"',
                  '\\"'
                );
                var tweetMediaUrl = this.getBetween(
                  data,
                  'data-image-url=\\"',
                  '\\"'
                );
                ...
                if (data.includes('<span class=\\"invisible')) {
                  tweetMessageFull = this.getBetween(
                    data,
                ...
                var tweetDetails = String(tweetMessageFull)
                  .replace(/<.*?>/g, '')
                  .replace(/(\\n)/gm, '')
                  .trim();
    ...
    if (matchedRegex !== undefined || matchedRegex !== null) {
      return text.replace(matchedRegex, function replace(match) {
        return String.fromCharCode(
          parseInt(match.replace(/&#x/g, '0x').replace(/;/g, ''), 16)
        );
      });
    } else {
      return text;
