          ...
          if (!tokens) {
            ...
            request.get(
              ...
              function (error, response, body) {
                if (error) {
                  console.error(`Error in connecting the YT $${error}`);
                  callback(error, null);
                } else {
  ...
  return new Promise((resolve, reject) => {
    this.reconnectGoogle(refreshToken, (error, tokens) => {
      if (error) {
        ...
        var url = `https://youtubeanalytics.googleapis.com/v2/reports?dimensions=day&endDate=${moment(
          untill
