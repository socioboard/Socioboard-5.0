      ...
      if (error) {
        ...
        requestPromise.get(`https://www.googleapis.com/youtube/v3/search?part=snippet&channelId=${channelId}&maxResults=50&type=video&access_token=${tokens.access_token}&type=video`)
          .then((response) => {
            ...
            if (parsedResponse.nextPageToken) return parsedResponse.nextPageToken;
  ...
  return new Promise((resolve, reject) => {
    this.reconnectGoogle(refreshToken, (error, tokens) => {
      if (error) {
        ...
        const url = `https://youtubeanalytics.googleapis.com/v2/reports?dimensions=day&endDate=${moment(untill).format('YYYY-MM-DD')}&ids=channel%3D%3D${socialId}&metrics=${this.google_api.youtube_insights_metrics}&sort=-day&startDate=${moment(since).format('YYYY-MM-DD')}&access_token=${tokens.access_token}`;
