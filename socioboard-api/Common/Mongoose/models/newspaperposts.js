...
newsPaperPost.methods.getNewsPaperArticles = function (
  urls = [],
  tags = [],
  author = '',
  startDate,
  ...
  return new Promise((resolve, reject) => {
    ...
    if (author != '' && urls.length > 0)
      condtion = [{mainUrl: {$in: urls}}, {author: author}];
    if (author != '' && tags.length > 0)
      condtion = [{tags: {$all: tags}}, {author: author}];

    if (tags.length > 0 && urls.length > 0)
      condtion = [{mainUrl: {$in: urls}}, {tags: {$all: tags}}];
    if (tags.length > 0 && author != '')
      condtion = [{tags: {$all: tags}}, {author: author}];

    if (author != '' && urls.length > 0 && tags.length > 0)
      condtion = [
        {mainUrl: {$in: urls}},
        {tags: {$all: tags}},
        {author: author},
...
newsPaperPost.methods.getNewsPaperStats = function (
  adminId,
  urls = [],
  tags = [],
  startDate,
...
newsPaperPost.methods.deleteTagsFromAnArticle = function (articleId, tags) {
  return new Promise((resolve, reject) => {
    return this.model('NewsPaperPosts')
      .findOneAndUpdate({_id: String(articleId)}, {$pullAll: {tags: tags}})
      .then(response => resolve(response))
      .catch(error => reject(error));
  });
};
