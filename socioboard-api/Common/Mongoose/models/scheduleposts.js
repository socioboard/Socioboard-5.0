...
schedulePosts.methods.getTeamSchedulerStats = function (teamId, since, until) {
  ...
  query = [
    {$match: query},
    {
      $project: {
        date: {
          $dateToString: {format: '%Y-%m-%d', date: '$normalScheduleDate'},
        },
      },
    },
    {
...
schedulePosts.methods.updateAdminResponse = function (
  scheduleId,
  adminResponse
) {
  return this.model('SchedulePost')
...
schedulePosts.methods.updateStatus = function (scheduleId, status) {
  return this.model('SchedulePost')
    .findOneAndUpdate({_id: String(scheduleId)}, {scheduleStatus: status})
    .then(result => {
      return result;
    })
    .catch(function (error) {
