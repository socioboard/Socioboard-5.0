...
rsssearchedurls.methods.updateTitle = function (post, userId, title) {
  return this.model('RssSearchedUrls')
    .update(
      {title: {$exists: false}, userId, rssUrl: post.rssUrl},
      {$set: {title}}
    )
    .then(() => {
      return '';
...
rsssearchedurls.methods.getBookMarkedPost = function (
  userId,
  skip = 0,
  limit = 0
) {
...
rsssearchedurls.methods.getSocialAccountPosts = function (
  channelId,
  skip,
  limit
) {
...
rsssearchedurls.methods.deleteRssUrls = function (userId, _id) {
  ...
  return this.model('RssSearchedUrls')
    ...
    .then(function (result) {
      ...
      else return 'Deleted Rss Url succesfully';
    })
    .catch(function (error) {
