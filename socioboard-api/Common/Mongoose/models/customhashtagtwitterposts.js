...
customhashtagtwitterposts.methods.getSocialAccountPostsForKeyword = function (
  accountId,
  keyword,
  skip,
  limit,
...
customhashtagtwitterposts.methods.deleteSingleTweet = function (tweetId) {
  ...
  return this.model('customhashtagtwitterposts')
    ...
    .then(function (result) {
      ...
      } else return 'Deleted tweet succesfully';
    })
    .catch(function (error) {
...
customhashtagtwitterposts.methods.getTwitterByCustomHashtag = function (
  keyword,
  skip,
  limit
) {
...
customhashtagtwitterposts.methods.returnmaxId = function (customhashtag) {
  return this.model('customhashtagtwitterposts')
    .find({customhashtag: customhashtag})
    .limit(1)
    .sort({publishedDate: 1})
    .then(function (result) {
      if (!result) {
...
customhashtagtwitterposts.methods.returnmaxIdWithDate = function (
  customhashtag
) {
  return this.model('customhashtagtwitterposts')
    .find({customhashtag: customhashtag})
...
customhashtagtwitterposts.methods.updateCommentCount = function (
  tweetId,
  method
) {
  var query = {
...
customhashtagtwitterposts.methods.findAllHashTags = function (
  startDate,
  endDate,
  hashtags,
  linkedHashtag,
  ...
  if (linkedHashtag) {
    if (custometag.length != 0) {
      ...
      custometag.map(customTag => {
        condition.push(
          {customtags: {$elemMatch: {groupname: customTag.groupname}}},
...
customhashtagtwitterposts.methods.getlinkedwithhashtag = function (
  startDate,
  endDate,
  hashtags,
  linkedHashtag,
...
customhashtagtwitterposts.methods.geteEmsSuggestedHashtag = function (
  startDate,
  endDate,
  hashtags,
  linkedHashtag,
...
customhashtagtwitterposts.methods.getTweetDayWiseold = function (
  social_id,
  isforTeam
) {
  var list = [];
...
customhashtagtwitterposts.methods.updateLikeedAccount = function (
  tweetId,
  accountId,
  accountName,
  followersCount,
...
customhashtagtwitterposts.methods.updateRetweetedaccount = function (
  tweetId,
  accountId,
  accountName,
  followersCount,
...
customhashtagtwitterposts.methods.getTweetDayWise = function (
  hashtags,
  startDate,
  endDate,
  sentiment,
...
customhashtagtwitterposts.methods.getSentimentAnalysis = function (
  hashtag,
  startDate,
  endDate,
  custometag
...
customhashtagtwitterposts.methods.getGenderAnalysisRand = function (
  social_id,
  isforTeam,
  hashtag,
  startDate,
...
customhashtagtwitterposts.methods.getCustomTagsFeeds = function (
  tweetId,
  groupName
) {
  var query = {};
...
customhashtagtwitterposts.methods.getGenderAnalysis = function (
  social_id,
  isforTeam,
  hashtag,
  startDate,
            ...
            for (var d in mentions) {
              ...
              request.get(
                ...
                function (error, response, body) {
                  ...
                  if (genType == 'female') female = female + 1;
                  else if (genType == 'male') male = male + 1;
...
customhashtagtwitterposts.methods.setSentimentStatus = function (
  tweetId,
  sentimentStatus
) {
  var query = {
  ...
  return this.model('customhashtagtwitterposts')
    ...
    .then(function (result) {
      if (!result) return 'No tweet found';
      else return 'Status changed succesfully';
    })
    .catch(function (error) {});
};

customhashtagtwitterposts.methods.setCustomTag = function (
  socialAccount,
  tweetId,
  customeTagValue
) {
  ...
  return this.model('customhashtagtwitterposts')
    ...
    .then(function (result) {
      if (!result) return 'No tweet found';
      else return 'Custome Tag Updated succesfully';
    })
    .catch(function (error) {});
};

customhashtagtwitterposts.methods.setCustomTags = function (
  tweetId,
  customeTagValue
) {
  var query = {
  ...
  return this.model('customhashtagtwitterposts')
    ...
    .then(function (result) {
      if (!result) return 'No tweet found';
      else return 'Custom Tag Updated succesfully';
    })
    .catch(function (error) {});
};

customhashtagtwitterposts.methods.setCustomTagsSpecificGroup = function (
  tweetId,
  customeTagValue
) {
  var query = {
    ...
    .then(function (result) {
      if (!result) {
        ...
        if (groupName) {
          ...
          if (checkCtag) {
            let checkCtag2 = result.customtags.filter(
...
customhashtagtwitterposts.methods.getArchivedAccountDetail = function (
  socialData
) {
  var list = [];
      ...
      .aggregate([
        ...
        {$unwind: '$accountId'},
        {
          $group: {
            _id: '$accountId',
            count: {$sum: 1},
            archivedStatus: {$addToSet: '$archivedStatus'},
          },
        },
      ])
...
customhashtagtwitterposts.methods.getSocialAccountPostsArray = function (
  accountId,
  skip,
  limit
) {
...
customhashtagtwitterposts.methods.getSingleCustomHashtagFeed = function (
  tweetId,
  skip,
  limit
) {
...
customhashtagtwitterposts.methods.getPrimitiveReport = function (
  statData,
  social_id,
  isforTeam,
  startDate,
...
customhashtagtwitterposts.methods.getPrimitiveReportStats = function (
  social_id,
  isforTeam,
  startDate,
  endDate
...
customhashtagtwitterposts.methods.getCustomHashtagAnalytics = function (
  hashtags,
  startDate,
  endDate,
  sentiment,
    ...
    this.model('customhashtagtwitterposts')
      ...
      .then(function (result) {
        ...
        for (var c in result) {
          ...
          if (result[c].isLiked) favorite = favorite + 1;

          if (result[c].isReplayTweet) replay = replay + 1;
          ...
          if (genders == 'male') totalMale = totalMale + 1;
          else if (genders == 'female') totalFemale = totalFemale + 1;
          else totalunknown = totalunknown + 1;
        }
...
customhashtagtwitterposts.methods.getSocialAccountSinglePostsLikeAndRetweetCount =
  function (tweetId) {
    var query = {
      tweetId: new RegExp(tweetId, 'i'),
    };
...
const customhashtagtwitterpostsModel = mongoose.model(
  'customhashtagtwitterposts',
  customhashtagtwitterposts
);
