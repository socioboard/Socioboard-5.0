...
class PdfMaker {
  ...
  setTableBody(bodyObj) {
    let widthType =
      this.tableHeaders.length < 10
        ? '*'
        : `${(100 / this.tableHeaders.length).toFixed(2)}%`;
    let tableFontSize =
      this.tableHeaders.length < 7
        ? 10
        : this.tableHeaders.length < 13
        ? 5
        : 4.5;
  ...
  setAppDomainList(pdfAppDomainList) {
    let max = Math.max(
      ...pdfAppDomainList.map(el => Math.max(...el.map(val => val.length)))
    );
  ...
  getTableBody(bodyObj) {
    ...
    for (let i = 0; i < bodyObj.length; i++) {
      ...
      for (let j = 0; j < this.tableHeadersOrder.length; j++) {
        arrValueInTableHeaderOrder.push(
          tableRow[this.tableHeadersOrder[j]] ?? ''
        );
      }
