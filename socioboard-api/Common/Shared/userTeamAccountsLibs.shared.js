...
import PublishedPostsMongoModel from '../Mongoose/models/publishedposts.js';
import TeamInsightsMongoModel from '../Mongoose/models/teamInsights.js';
      ...
      return teamSocialAccountJoinTable
        ...
        .then(result => {
          if (result) resolve();
            ...
            throw new Error(
              'Account isnt belongs to team or account is locked for the team!'
            );
        })
        .catch(error => {
          reject(error);
    ...
    return new Promise((resolve, reject) => {
      ...
      return this.getUserTeams(userId)
        ...
        .then(accountTeam => {
          ...
          resolve({
            isValid: intersectTeams.length > 0 ? true : false,
            intersectTeams: intersectTeams,
          });
        })
        .catch(error => {
          reject(error);
  ...
  getSocialAccount(accountType, accountId, userId, teamId) {
    return new Promise((resolve, reject) => {
      if (!accountType || !accountId || !userId || !teamId) {
        reject(
          new Error('Please verify your inputs: 1. Account id, \n\r 2.Team id')
        );
      } else {
      ...
      if (!teamId) throw new Error('TeamId cant be null');
        ...
        return teamInfo
          ...
          .then(teamMembersData => {
            ...
            return Promise.all(
              teamDetails.map(accounts => {
                socialProfiles = accounts.SocialAccount.length;
                return Promise.all(
                  accounts.SocialAccount.map(account => {
                    let fields = [];
                    ...
                    if (fields.length > 0) {
                      return updateFriendsTable
                        ...
                        .then(resultData => {
                          ...
                          switch (Number(account.account_type)) {
                            case 1:
                              SocialAccountStats.facebookStats.push({
                                facebookStats: data,
                              });
                              break;
                            case 4:
                              SocialAccountStats.twitterStats.push({
                                twitterStats: data,
                              });
                              break;
                            case 5:
                              SocialAccountStats.instagramStats.push({
                                instagramStats: data,
                              });
                              break;
                            case 9:
                              SocialAccountStats.youtubeStats.push({
                                youtubeStats: data,
                              });
                              break;
                            default:
          ...
          .then(publishedCount => {
            ...
            if (!update || update == null || update == false)
              return teamInsightsMongoModelObject.insertInsights(datas);
            else
              return teamInsightsMongoModelObject.addTeamInsights(
                teamId,
                updatedData
              );
          })
