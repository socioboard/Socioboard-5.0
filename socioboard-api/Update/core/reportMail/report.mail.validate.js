...
class ReportMailValidate {
    ...
    validAutoReport(data) {
        const JoiSchema = Joi.object({
            ...
            frequency: Joi.number().required().min(0).max(2).messages({ 'any.required': "Frequency required", 'number.min': "Frequency should be 0, 1 or 2", 'number.max': "Frequency should be 0, 1 or 2" }),
            report: Joi.number().required().min(0).max(2).messages({ 'any.required': "Report type required", 'number.min': "Report should be 0, 1 or 2", 'number.max': "Report should be 0, 1 or 2" }),
