    ...
    if (decodedToken) {
      ...
      if (remindingDays < 0) {
        ...
        if (redirectValueFromRequest) {
          ...
          res.redirect(`${config.get('update_socioboard.host_url')}/v1/user/change-plan/?userId=${parsedToken.user_id}&currentPlan=${parsedToken.Activations.user_plan}&newPlan=0&redirectToken=${redirectToken}`);
        }
      } else {
        next();
      }
