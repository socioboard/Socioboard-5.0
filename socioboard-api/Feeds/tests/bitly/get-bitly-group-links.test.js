...
import { GET_BITLY_GROUP_LINKS, SERVER } from '../constants.js';
...
describe('Get bitly platform limits test', () => {
  let accessToken = '';
  let bitlyAccount;
  let teamId;

  const user = {
    username: 'testGetBitlyGroupLinks',
    email: 'testGetBitlyGroupLinks@testtesttestabcd.com',
  ...
  test('Positive', async () => {
    await request(SERVER)
      ...
      .query({
        ...
        teamId,
        group_guid: BITLY_CONSTANTS.GROUP_GUID_FORBIDDEN,
      })
      .expect(200)
      .expect(({ body }) => expect(body.code).toBe(400));
  });
  ...
  test('Positive', async () => {
    await request(SERVER)
      ...
      .query({
        accountId: bitlyAccount.profileDetails.account_id,
        teamId,
        group_guid: BITLY_CONSTANTS.GROUP_GUID_VALID,
      })
      .expect(200)
