...
class FeedsService {
  ...
  async getFacebookFeeds(req, res, next) {
    ...
    try {
      ...
      const socialAccountDetails = await userTeamAccounts.getSocialAccount([1, 2, 3], accountId, req.body.userScopeId, teamId);
      const offset = (pageId - 1) * config.get('perPageLimit');
      const facebookMongoPostModelObject = new FacebookMongoPostModel();
      const feeds = await facebookMongoPostModelObject.getSocialAccountPosts(socialAccountDetails.social_id, offset, config.get('perPageLimit'));
  ...
  async facebookLike(req, res, next) {
    ...
    try {
      ...
      const response = await feedsLibs.facebookLike(req.body.userScopeId, accountId, teamId, postId);
  ...
  async facebookComment(req, res, next) {
    ...
    try {
      const {
        teamId, accountId, postId, comment,
      } = req.query;
      ...
      const response = await feedsLibs.facebookComment(req.body.userScopeId, req.query.accountId, req.query.teamId, req.query.postId, req.query.comment);
  ...
  async getRecentFbFeeds(req, res, next) {
    ...
    try {
      ...
      if (error) return ValidateErrorResponse(res, error.details[0].message);
      const socialAccountDetails = await userTeamAccounts.getSocialAccount([1, 2, 3], accountId, req.body.userScopeId, teamId);
      const response = await feedsLibs.getRecentFbFeeds(req.body.userScopeId, req.query.accountId, req.query.teamId, req.query.pageId);
      const SocialAccountStats = await feedsLibs.socialAccountStats(accountId);
  ...
  async getRecentFbPageFeeds(req, res, next) {
    try {
      ...
      if (error) return ValidateErrorResponse(res, error.details[0].message);
      const socialAccountDetails = await userTeamAccounts.getSocialAccount([1, 2, 3], accountId, req.body.userScopeId, teamId);
      const response = await feedsLibs.getRecentFbPageFeeds(req.body.userScopeId, req.query.accountId, req.query.teamId, req.query.pageId);
      const SocialAccountStats = await feedsLibs.socialAccountStats(accountId);
      const data = { socialAccountDetails, SocialAccountStats, feeds: response };
