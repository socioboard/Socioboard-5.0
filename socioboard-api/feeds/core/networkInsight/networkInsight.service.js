import NetworkInsightValidate from './networkInsight.validate.js'
import { ValidateErrorResponse, SuccessResponse, CatchResponse } from '../../../Common/Shared/response.shared.js'
import moment from 'moment'
import UserTeamAccountLibs from '../../../Common/Shared/userTeamAccountsLibs.shared.js';
import TeamInsights from '../../../Common/Mongoose/models/teamInsights.js'
import TwitterInsights from '../../../Common/Mongoose/models/twitterInsights.js'
...
class NetworkService {
    ...
    async getTeamInsights(req, res, next) {
        try {
            ...
            let dates = await this.getFilteredPeriod(filterPeriod, req.query.since, req.query.until)
            let checkTeamValid = await this.userTeamAccountLibs.isTeamValidForUser(req.body.userScopeId, teamId)
            let getUpdateTeamDetail = await this.userTeamAccountLibs.createOrUpdateTeamReport(teamId, false)
            let response = await this.teamInsights.getInsights(teamId, dates.since, dates.untill)
            if (response.length <= 0) throw new Error("Sorry, No Team-Reports available in this Time Period")
            let InsightResult = response;
            //res.send(response)
            let twitterInsights = {};
            let instagramBusinessInsights = {};
            let facebookInsights = {};
            let youtubeInsights = {};
            let teamMembstats = {};
            let Facebook = [];
            let Twitter = [];
            let InstagramBusiness = [];
            let Youtube = [];
    ...
    async getTwitterInsights(req, res, next) {
        try {
            ...
            let getSocialAccDetails = await this.userTeamAccountLibs.getSocialAccount(4, accountId, req.body.userScopeId, teamId)
            let dates = await this.getFilteredPeriod(filterPeriod, req.query.since, req.query.until)
            let twitterInsight = await this.twitterInsights.getInsightsDaywise(accountId, dates.since, dates.untill)
            let data = await this.formatDate(twitterInsight, dates.since, dates.untill)
            let stats = await this.twitterInsights.getInsightsStats(accountId, dates.since, dates.untill)
    ...
    async getTwitterStats(req, res, next) {
        try {
            ...
            let getSocialAccDetails = await this.userTeamAccountLibs.getSocialAccount(4, accountId, req.body.userScopeId, teamId)
            let dates = await this.getFilteredPeriod(filterPeriod, req.query.since, req.query.until)
            let response = await this.twitterInsights.getInsightsStats(accountId, dates.since, dates.untill)
    ...
    async getFacebookInsights(req, res, next) {
        try {
            ...
            let getSocialAccDetails = await this.userTeamAccountLibs.getSocialAccount(2, accountId, req.body.userScopeId, teamId)
            let dates = await this.getFilteredPeriod(filterPeriod, req.query.since, req.query.until)
    ...
    async getYouTubeInsights(req, res, next) {
        try {
            ...
            let getSocialAccDetails = await this.userTeamAccountLibs.getSocialAccount(9, accountId, req.body.userScopeId, teamId)
            let dates = await this.getFilteredPeriod(filterPeriod, req.query.since, req.query.until) //modification required
            let data = await this.googleHelper.youtubeInsights(getSocialAccDetails.refresh_token, encodeURIComponent(getSocialAccDetails.social_id), dates.since, dates.untill)
    ...
    async getLinkedInInsights(req, res, next) {
        try {
            ...
            let getSocialAccDetails = await this.userTeamAccountLibs.getSocialAccount(9, accountId, req.body.userScopeId, teamId)
            let dates = await this.getFilteredPeriod(filterPeriod, req.query.since, req.query.until) //modification required
            let response = await this.LinkedInHelper.getCompanyInsights(getSocialAccDetails.access_token, encodeURIComponent(getSocialAccDetails.social_id), dates.since, dates.untill)
            response ? SuccessResponse(res, response) : CatchResponse(res, "Error in fethcing the Youutbe Insight")
