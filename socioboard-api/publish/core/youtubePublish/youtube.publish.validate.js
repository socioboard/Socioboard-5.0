...
class YouTubeUploadValidator {
    ...
    validateUploadVideo(data) {
        const JoiSchema = Joi.object({
            teamId: Joi.string().required().messages({ 'any.required': "Team id required to upload" }),
            accountId: Joi.string().required().messages({ 'any.required': "Select account id for upload" }),
            postDetails: Joi.object().required(),
            postDetails: Joi.object({
                mediaUrls: Joi.array().min(1).required().messages({ 'any.required': "Media required to upload", 'array.min': "Media required to upload" }),
                postType: Joi.number().min(0).max(1).required().messages({ 'any.required': "Post type required to upload", 'number.min': "Post type should be 0 or 1", 'number.max': "Post type should be 0 or 1" }),
                resource: Joi.object().required().messages({ 'any.required': "Video resource details required" }),
                resource: Joi.object({
                    snippet: Joi.object({
                        title: Joi.string().required().messages({ 'any.required': "Title for the video is required", 'string.empty': 'Title not allowed to be empty' }),
                        description: Joi.string().allow(''),
                        tags: Joi.array().min(1).messages({ 'array.min': "At least one tag required" }),
                    ...
                    status: Joi.object({
                        privacyStatus: Joi.string().valid('private', 'public').required().messages({
                        ...
                        publishAt: Joi.when('privacyStatus', {
                            ...
                            'any.only': "Privacy status must be private for schedule date and time to make video public"
                        })
                    })
                })
            })
    ...
    validateUploadVideoDetails(data) {
        const JoiSchema = Joi.object({
            ...
            accountId: Joi.string().required().messages({ 'any.required': "Account id required" }),
            pageId: Joi.string().required().messages({ 'any.required': "Page Id id required" }),
            postType: Joi.number().required().min(0).max(2).messages({ 'any.required': "Post type id required", 'number.min': "Post type should be 0, 1 or 2", 'number.max': "Post type should be 0, 1 or 2" }),
        }).options({ abortEarly: false });
        return JoiSchema.validate(data)
    }
