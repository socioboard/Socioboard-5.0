<nav class="navbar fixed-top navbar-expand-lg navbar-light bg-white ">
    <div class="container">
        <a class="navbar-brand"
           href="{{env('APP_URL')}}dashboard/{{session()->get('team')['teamSocialAccountDetails'][0][0]->team_id}}">
            ...
            @if(session()->get('user')['userDetails']->userPlanDetails->is_plan_active )
                <ul class="navbar-nav mr-auto">
                    <li class="nav-item active">
                        <a class="nav-link"
                           href="{{env('APP_URL')}}dashboard/{{session()->get('team')['teamSocialAccountDetails'][0][0]->team_id}}">Dashboard
                    ...
                    <li class="nav-item dropdown">
                        @if(session()->get('user')['userDetails']->userPlanDetails->content_studio == 1)
                            ...
                            <div class="dropdown-menu" aria-labelledby="ContentStudio">
                                ...
                                <a class="dropdown-item" href="{{env('APP_URL')}}content-studio/dailymotion"
                                   title="Dailymotion">Dailymotion</a>
                                <a class="dropdown-item" href="{{env('APP_URL')}}content-studio/newsapi"
                                   title="NewsApi">NewsApi</a>
                                <a class="dropdown-item" href="{{env('APP_URL')}}content-studio/pixabay"
                                   title="Pixabay">Pixabay</a>
                        ...
                        @else
                            <a class="nav-link dropdown-toggle" href="#" id="discoveryNavbarDropdown" role="button"
                               data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"
                               onclick="planCheck({{session()->get('user')['userDetails']->userPlanDetails->content_studio}})">
                                Content Studio
                            </a>
                        @endif
                    </li>
                    <li class="nav-item dropdown">
                        ...
                        @if(session()->get('user')['userDetails']->userPlanDetails->discovery == 1)
                            <a class="nav-link dropdown-toggle" href="#" id="boardNavbarDropdown" role="button"
                        ...
                        @else
                            <a class="nav-link dropdown-toggle" href="#" id="boardNavbarDropdown" role="button"
                               data-toggle="dropdown"
                               aria-haspopup="true" aria-expanded="false"
                               onclick="planCheck({{session()->get('user')['userDetails']->userPlanDetails->discovery}})">
                                Discovery
                            </a>
                        @endif
                    </li>
                    <li class="nav-item dropdown">
                        @if(session()->get('user')['userDetails']->userPlanDetails->rss_feeds == 1)
                            ...
                            <div class="dropdown-menu" aria-labelledby="rssDrop">
                                <a class="dropdown-item" href="{{env('APP_URL')}}discovery/rss-feed"
                                   title="Content Feeds"> Content Feeds</a>
                            </div>
                        @else
                            <a class="nav-link dropdown-toggle" href="#" id="rssDrop" role="button"
                               onclick="planCheck({{session()->get('user')['userDetails']->userPlanDetails->rss_feeds}})"
                               data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                RSS
                            </a>
                        @endif
                    ...
                    <li class="nav-item">
                        ...
                        @else
                            <a class="nav-link" href="#"
                               onclick="planCheck({{session()->get('user')['userDetails']->userPlanDetails->board_me}})">BoardMe</a>
                        @endif
                    </li>
                        ...
                        @if(session()->get('user')['userDetails']->userPlanDetails->social_report == 1)
                            ...
                            <div class="dropdown-menu" aria-labelledby="ReportDropdown">
                                ...
                                <a class="dropdown-item"
                                   href="{{env('APP_URL')}}team-report/{{session()->get('currentTeam')['team_id']}}"><strong>{{session()->get('currentTeam')['team_name']}}</strong>
                                    Team Report</a>
                                {{--                            <a class="dropdown-item" href="{{env('APP_URL')}}/report/{{env('REPORT_INITIAL')}}/{{env('FACEBOOKPAGE')}}" title="Facebook">Facebook--}}
                                {{--                                Page</a>--}}
                                <a class="dropdown-item"
                                   href="{{env('APP_URL')}}/report/{{env('REPORT_INITIAL')}}/{{env('TWITTER')}}"
                                   title="Twitter">Twitter</a>
                                {{--                            <a class="dropdown-item" href="{{env('APP_URL')}}/report/{{env('REPORT_INITIAL')}}/{{env('INSTAGRAMBUSINESSPAGE')}}" title="Instagram">Instagram--}}
                                {{--                                Business Account</a>--}}
                                <a class="dropdown-item"
                                   href="{{env('APP_URL')}}/report/{{env('REPORT_INITIAL')}}/{{env('YOUTUBE')}}"
                                   title="YouTube">YouTube</a>
                            </div>
                        @else
                            <a class="nav-link dropdown-toggle" href="#" id="ReportDropdown"
                               onclick="planCheck({{session()->get('user')['userDetails']->userPlanDetails->social_report}})">Report</a>
                        @endif
                    ...
                    <li class="nav-item dropdown">
                        @if(session()->get('user')['userDetails']->userPlanDetails->share_library == 1)
                            ...
                            <div class="dropdown-menu" aria-labelledby="ReportDropdown">
                                <a class="dropdown-item" href="{{env('APP_URL')}}image-library/1"
                                   title="Private Image Library">Private Image Library</a>
                                <a class="dropdown-item" href="{{env('APP_URL')}}image-library/0"
                                   title="Public Image Library">Public Image Library</a>
                            </div>
                        @else
                            <a class="nav-link dropdown-toggle" href="#" id="ReportDropdown"
                               onclick="planCheck({{session()->get('user')['userDetails']->userPlanDetails->share_library}})">Image
                                Library</a>
                        @endif
                ...
                <ul class="navbar-nav">
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle dropdown-toggle-none-c" href="#" id="teamNavbarDropdown"
                           role="button"
                           data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                            <i class="fas fa-chalkboard-teacher text-primary" data-toggle="tooltip"
                               data-placement="bottom" title="Teams"></i>
                        </a>
                        @if(session()->has('team'))
                            {{--$oldTeamId--}}
                            <?php
                            $old_id = session()->get('currentTeam')['team_id'];
                            $user_id = session('user')['userDetails']->user_id;
                            ?>
                            <div class="dropdown-menu" aria-labelledby="teamNavbarDropdown">
                                @for ($i = 0; $i < count(session()->get('team')['teamSocialAccountDetails']); $i++)

                                    <a class="dropdown-item"
                                       href="{{env('APP_URL')}}dashboard/{{session()->get('team')['teamSocialAccountDetails'][$i][0]->team_id}}"
                                       onclick="sendId('{{session()->get('team')['teamSocialAccountDetails'][$i][0]->team_id}}','{{$old_id}}','{{$user_id}}')"
                                       id="{{session()->get('team')['teamSocialAccountDetails'][$i][0]->team_id}}">{{session()->get('team')['teamSocialAccountDetails'][$i][0]->team_name}}</a>

                                @endfor
                                <input class="oldTeamId" type="hidden"
                                       id="{{session()->get('currentTeam')['team_id']}}">
                                <div class="dropdown-divider"></div>
                                <a class="dropdown-item" href="{{env('APP_URL')}}accept-invitation">Invitation
                                    Request</a>
                                <a class="dropdown-item"
                                   href="{{env('APP_URL')}}view-team/{{session()->get('currentTeam')['team_id']}}"
                                   id="">View Team</a>
                ...
                <ul class="navbar-nav">
                    ...
                    <li class="nav-item dropdown">
                        ...
                        <div class="dropdown-menu" aria-labelledby="settingsNavbarDropdown">
                            ...
                            <div class="dropdown-divider"></div>
                            <a class="dropdown-item" href="{{env('APP_URL')}}logout">Logout <i
                                    class="fas fa-sign-out-alt text-primary float-right"></i></a>
                        </div>
                    </li>
                </ul>
            @endif
...
<script>
    //on team change
    function sendId(x, y, z) {
        var currentTeamId = Number(x);
        var oldTeamId = Number(y);
        var userId = Number(z);
        if (currentTeamId !== oldTeamId) {
            var teamChangeData = {newTeamId: currentTeamId, oldTeamId: oldTeamId, userId: userId};
            socket.emit('teamRemove', teamChangeData.oldTeamId);
            socket.emit('teamAdding', teamChangeData.newTeamId);
        }
